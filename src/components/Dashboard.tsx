...
import { Activity, Shield, Search, FlaskConical, GitBranch, Database, Terminal, KeyRound, Lock, Layers, Crosshair } from 'lucide-react';
...
const defaultStats: Stats = {
  scopes: 0, endpoints: 0, payloads: 0, flows: 0,
  sessions: 0, credentials: 0, scans: 0, runningScans: 0,
  automationJobs: 0, runningJobs: 0, findings: 0, authFlows: 0, requests: 0,
};
...
export default function Dashboard() {
  ...
  const cards = [
    ...
    { label: 'Endpoints', value: stats.endpoints, icon: Search, color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/30' },
    ...
    { label: 'Auth Sessions', value: stats.sessions, icon: KeyRound, color: 'text-sky-400', bg: 'bg-sky-500/10', border: 'border-sky-500/30' },
    { label: 'Credentials', value: stats.credentials, icon: Lock, color: 'text-fuchsia-400', bg: 'bg-fuchsia-500/10', border: 'border-fuchsia-500/30' },
    { label: 'Scans', value: stats.scans, sub: stats.runningScans > 0 ? `${stats.runningScans} running` : undefined, icon: Crosshair, color: 'text-orange-400', bg: 'bg-orange-500/10', border: 'border-orange-500/30' },
    { label: 'Findings', value: stats.findings, icon: Layers, color: 'text-red-400', bg: 'bg-red-500/10', border: 'border-red-500/30' },
  ] as const;
        ...
        {cards.map((card, i) => (
          <div key={i} className={`cyber-card ${card.border} p-6 flex flex-col gap-4 group hover:bg-emerald-900/20 transition-all duration-300`}>
            ...
            <div className="relative z-10">
              ...
              {'sub' in card && card.sub && <div className="text-[9px] text-amber-400/70 font-mono animate-pulse">{card.sub}</div>}
            </div>
          </div>
        ))}
      </div>
