...
import { Activity, Play, AlertCircle, CheckCircle2, Terminal, RefreshCw, Wand2, ChevronDown, ChevronUp, Trash2, Eye, EyeOff } from 'lucide-react';
...
export default function Scanner({ initialUrl }: { initialUrl?: string }) {
  ...
  const viewResponseBody = async (resultId: string, responseId: string | null) => {
    if (expandedResult === resultId) {
      setExpandedResult(null);
      return;
    }
    setExpandedResult(resultId);
    if (responseId && !responseBody[resultId]) {
      try {
        const res = await fetch(`/api/history`);
        const history = await res.json();
        const match = history.find((h: any) => h.id === responseId);
        if (match) {
          setResponseBody(prev => ({ ...prev, [resultId]: match.res_body || '(empty body)' }));
        } else {
          setResponseBody(prev => ({ ...prev, [resultId]: '(response not found in history)' }));
        }
      } catch {
        setResponseBody(prev => ({ ...prev, [resultId]: '(failed to load response)' }));
      }
    }
  };
                ...
                <tbody className="divide-y divide-emerald-900/20">
                  {results.map(res => (
                    <React.Fragment key={res.id}>
                      <tr className={`hover:bg-emerald-950/30 transition-colors group ${res.is_anomaly ? 'bg-red-500/5' : ''}`}>
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-2">
                            ...
                            {res.response_id && (
                              <button
                                onClick={() => viewResponseBody(res.id, res.response_id)}
                                className="ml-2 text-emerald-500/50 hover:text-emerald-400 transition-colors"
                                title="View response body"
                              >
                                {expandedResult === res.id ? <EyeOff className="w-3 h-3" /> : <Eye className="w-3 h-3" />}
                              </button>
                            )}
                          </div>
                        </td>
                      ...
                      {expandedResult === res.id && (
                        <tr>
                          <td colSpan={4} className="p-0">
                            <pre className="p-4 bg-black/60 text-[10px] text-emerald-200 overflow-auto max-h-48 border-t border-emerald-900/20">
                              {responseBody[res.id] || '(loading...)'}
                            </pre>
                          </td>
                        </tr>
                      )}
