      ...
      <header className="mb-6 flex justify-between items-end relative z-10">
        ...
        <div className="flex gap-2">
          {history.length > 0 && (
            <button
              onClick={async () => {
                if (!confirm('Clear ALL request/response history? This cannot be undone.')) return;
                await fetch('/api/history', { method: 'DELETE' });
                setHistory([]);
                setFilteredHistory([]);
                setSelectedItem(null);
              }}
              className="p-2 bg-red-900/20 border border-red-800/50 rounded-md hover:bg-red-900/40 text-red-400/70 hover:text-red-400 transition-all"
